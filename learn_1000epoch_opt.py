...
# opt HP
opt ={'band_gap': {'n_conv': 6, 'atom_fea_len': 139, 'h_fea_len': 5, 'n_h': 2},
...
SPACE = [
    space.Integer(3,4, name='n_conv', prior='uniform'),
    space.Integer(64, 65, name='atom_fea_len',prior='uniform'),
    space.Integer(127, 128, name='h_fea_len'),
    space.Integer(1, 2, name='n_h'),]
...
for k,v in reference_csv.items():
    for prop in v:
        ...
        set_property_to_ids(df, prop)
        free_cache = True
        results = skopt.gp_minimize(objective, SPACE, n_calls=n_calls, n_initial_points=1)
